...
loadProducts(renderProductsGrid); //save the renderProductsGrid() function as a parameter to be used by  the loadProducts(renderProductsGrid) function. This function will be used by loadProducts() and be saved as a parameter as  loadProducts(fun) in data/products.js file. This means that when loadproducts() function runs, it will wait for all the html under renderProductsGrid()to run before now lopping through it to display the product on the webpage. if this not done, the loadproducts() will run and it wont wait for the html to load which will result in a blank page. Note, i later called  back fun as a function inside the loadProducts() in data/products.js file.
